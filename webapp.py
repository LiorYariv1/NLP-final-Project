...
class Pyweb():
    ...
    def ui(self):
        ...
        put_markdown('Insert A Few Keywords (you dont have to use all the boxes)')
        put_row([pin.put_input(f'kw_{i}') for i in range(self.num_kw)])
        put_button('GENERATE', onclick=self.generate)
        put_button('Random', onclick = self.test_plots)
        put_button('clear input', onclick = self.clear_widget)
    ...
    def test_plots(self):
        ...
        with pywebio.output.use_scope(cur_scope):
            ...
            put_row([put_markdown(f'<b> Movie Title</b>: {title}  <br> <b>{genres_txt}:</b> {genre} <br><b>key words:</b> {kw}')])
            res = sample['full_model_9_beams'].item().replace('title2008-10-08workNY Times','')
            put_markdown(f'<b> Your Generated Movie Plot </b> <br> {res}')
...
if __name__ == '__main__':
    pyweb = Pyweb()
    pywebio.start_server(pyweb.ui, port=8777,remote_access=True)